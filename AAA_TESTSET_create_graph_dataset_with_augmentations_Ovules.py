...
# %%
graphs_list = []
for img_name in Ovules_data_dict_test.keys():
    print(img_name)
    ...
    for img_trans in range(num_augmentations):
        ...
        hf = np.load(Ovules_data_dict_test[img_name])
        ...
        raw_img = torch.from_numpy(output['raw'])
        hand_seg = torch.from_numpy(output['handseg'])
