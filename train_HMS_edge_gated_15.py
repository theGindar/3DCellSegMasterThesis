...
from func.network import VoxResNet, CellSegNet_basic_lite, CellSegNet_basic_edge_gated_deep_supervised
...
# hyperparameters
# ----------
save_path = 'output/model_HMS_edge_gated_deep_supervised_15.pkl'
need_resume = True
load_path = 'output/model_HMS_edge_gated_deep_supervised_15.pkl'
loss_save_path = 'output/loss_HMS_edge_gated_deep_supervised_15.csv'
learning_rate = 1e-4
max_epoch = 500
model_save_freq = 20
...
# init model
model=CellSegNet_basic_edge_gated_deep_supervised(input_channel=1, n_classes=3, output_func = "softmax")
...
for ith_epoch in range(0, max_epoch):
    for ith_batch, batch in enumerate(dataset_loader):
        ...
        seg_output, e_output, seg_output_8, seg_output_16, seg_output_32 = model(img_input)
        ...
        loss_32 = dice_loss_org_weights(seg_output_bb_32, seg_groundtruth_bb, weights_bb) + \
                  dice_loss_II_weights(seg_output_f_32, seg_groundtruth_f, weights_f)
        # accuracy_32 = dice_accuracy(seg_output_f_32, seg_groundtruth_f_32)

        loss_16 = dice_loss_org_weights(seg_output_bb_16, seg_groundtruth_bb, weights_bb) + \
                  dice_loss_II_weights(seg_output_f_16, seg_groundtruth_f, weights_f)
        # accuracy_16 = dice_accuracy(seg_output_f_16, seg_groundtruth_f_16)

        loss_8 = dice_loss_org_weights(seg_output_bb_8, seg_groundtruth_bb, weights_bb) + \
                 dice_loss_II_weights(seg_output_f_8, seg_groundtruth_f, weights_f)
        ...
        loss = (2*loss_1 + loss_2 + loss_32 + loss_16 + loss_8) / 6
