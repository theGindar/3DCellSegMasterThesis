...
from func.network import VoxResNet, CellSegNet_basic_lite_w_groupnorm_deep_supervised_VIII
...
# init model
model=CellSegNet_basic_lite_w_groupnorm_deep_supervised_VIII(input_channel=1, n_classes=3, output_func = "softmax")
...
for ith_epoch in range(0, max_epoch):
    ...
    if (ith_epoch+1)%model_save_freq==0:
        ...
        torch.save({'model_state_dict': model.state_dict()}, f'output/model_HMS_w_groupnorm_batchsize5_deep_supervision_8.pkl')
        model.to(device)
