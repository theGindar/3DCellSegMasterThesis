...
from func.loss_func import dice_accuracy, dice_loss_II, dice_loss_II_weights, dice_loss_org_weights, cross_entropy_with_weights
...
# hyperparameters
# ----------
save_path = 'output/model_HMS_w_groupnorm_new_loss.pkl'
need_resume = True
...
for ith_epoch in range(0, max_epoch):
    for ith_batch, batch in enumerate(dataset_loader):
        ...
        seg_groundtruth_boundary = torch.tensor(batch['boundary']>0, dtype=torch.float)
        seg_groundtruth_foreground_edge = torch.tensor(batch['edge_foreground']>0, dtype=torch.float)
        ...
        loss = loss_cellseg + 0.5 * boundary_loss
        ...
        print(
            ...
            "time(s) {time:.2f}\t"
            "loss {loss:.5f}\t"
            "loss_cellseg {loss_cellseg:.5f}\t"
            "boundary_loss {boundary_loss:.5f}\t"
            "acc {acc:.5f}\t".format(
                ith_epoch + 1,
                max_epoch,
                ith_batch,
                ...
                loss = loss.item(),
                loss_cellseg=loss_cellseg.item(),
    ...
    if (ith_epoch+1)%model_save_freq==0:
        ...
        torch.save({'model_state_dict': model.state_dict()}, f'output/model_HMS_w_groupnorm_new_loss.pkl')
        model.to(device)
