...
# hyperparameters
# ----------
save_path = 'output/model_HMS_6.pkl'
need_resume = True
load_path = 'output/model_HMS_6.pkl'
loss_save_path = 'output/loss_HMS_6.csv'
learning_rate = 1e-4
max_epoch = 500
model_save_freq = 20
...
for ith_epoch in range(0, max_epoch):
    for ith_batch, batch in enumerate(dataset_loader):
        ...
        seg_centroid_foreground_groundtruth = torch.tensor(batch['centroid_foreground'], dtype=torch.float).to(device)
        ...
        e_output_edge = torch.cat((e_output[:,0,:,:,:], e_output[:,1,:,:,:]), dim=1)
        ...
        # TODO change!
        loss_2 = ((dice_loss_org_individually(e_output_edge, groundtruth_target) + \
                 .5 * balanced_cross_entropy(e_output_edge, groundtruth_target))*2 + \
                 mse_loss(e_output[:,2,:,:,:], seg_centroid_foreground_groundtruth))/3
