...
from func.network import VoxResNet, CellSegNet_basic_lite_w_groupnorm_deep_supervised_X
...
# init model
model=CellSegNet_basic_lite_w_groupnorm_deep_supervised_X(input_channel=1, n_classes=3, output_func = "softmax")
...
for ith_epoch in range(0, max_epoch):
    for ith_batch, batch in enumerate(dataset_loader):
        ...
        loss_16 = dice_loss_org_weights(seg_output_bb_16, seg_groundtruth_bb_64, weights_bb_64) + \
                  dice_loss_II_weights(seg_output_f_16, seg_groundtruth_f_64, weights_f_64)
        # accuracy_16 = dice_accuracy(seg_output_f_16, seg_groundtruth_f_16)

        loss_8 = dice_loss_org_weights(seg_output_bb_8, seg_groundtruth_bb_64, weights_bb_64) + \
        ...
        if ith_epoch <= 300:
            loss = (loss_merged + loss_64 + loss_32 + loss_16 + loss_8) / 5
        else:
            loss = (5*loss_merged + loss_64 + loss_32 + loss_16 + loss_8) / 9
    ...
    if (ith_epoch+1)%model_save_freq==0:
        ...
        torch.save({'model_state_dict': model.state_dict()}, f'output/model_HMS_w_groupnorm_batchsize5_deep_supervision_10.pkl')
        model.to(device)
