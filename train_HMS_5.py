# train
# train with other dice loss (with foreground weights)
...
for ith_epoch in range(0, max_epoch):
    for ith_batch, batch in enumerate(dataset_loader):
        ...
        groundtruth_target_bb = torch.cat((seg_edge_background_groundtruth,
        ...
        e_weights_f = batch['weights_edge_foreground'].to(device)
        e_weights_bb = torch.cat((batch['weights_edge_background'], batch['weights_edge']), dim=1).to(device)
