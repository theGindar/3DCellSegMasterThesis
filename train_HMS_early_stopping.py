...
val_gt = torch.tensor(final_gt_stack, dtype=torch.float).to(device)

val_input_np = np.stack((raw_val_0,
                      raw_val_1,
                      raw_val_2,
                      raw_val_3,
                      raw_val_4), axis=0)

val_input_np = np.expand_dims(val_input_np, axis=1)
val_input = torch.tensor(val_input_np, dtype=torch.float).to(device)
...
for ith_epoch in range(0, max_epoch):
    ...
    if (ith_epoch+1)%model_save_freq==0:
        seg_val_output = model(val_input)
        accuracy_val = dice_accuracy(seg_val_output, val_gt)
        if accuracy_val >= best_val_acc:

            print(f"new best vall acc: {accuracy_val}")
            ...
            print(f"vall acc got worse: {accuracy_val}")
